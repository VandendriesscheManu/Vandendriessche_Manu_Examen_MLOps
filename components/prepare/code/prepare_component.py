...
def main():
    ...
    # locate data file (MLTable-safe: parquet preferred, else csv)
    parquets = glob(os.path.join(args.input_folder, "*.parquet"))
    csvs = glob(os.path.join(args.input_folder, "*.csv"))

    if parquets:
        print("📦 Reading parquet file:", parquets[0])
        df = pd.read_parquet(parquets[0])
    elif csvs:
        print("📄 Reading CSV file:", csvs[0])
        df = pd.read_csv(csvs[0])
    else:
        raise FileNotFoundError(f"No parquet or CSV found in input folder: {args.input_folder}")
